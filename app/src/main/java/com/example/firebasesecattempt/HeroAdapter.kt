package com.example.firebasesecattempt
...
class HeroAdapter(val mCtx: Context, val layoutResId: Int, val heroList: List<Hero>):ArrayAdapter<Hero>(mCtx, layoutResId, heroList){
    ...
    private fun showUpdateDialog(hero: Hero) {
        ...
        builder.setPositiveButton("Update") { dialog, which ->
            ...
            dbHero.child(hero.id.toString()).setValue(hero)

            Toast.makeText(mCtx, "Hero Updated", Toast.LENGTH_LONG).show()
        }

        builder.setNegativeButton("Delete") { dialog, which ->
            val myDatabase = FirebaseDatabase.getInstance().getReference("heroes")
            myDatabase.child(hero.id.toString()).removeValue()
        }
